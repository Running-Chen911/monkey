...
// @match        http://www.icourse163.org/spoc/newExam/*
// @match        https://www.icourse163.org/spoc/newExam/*
// @match        https://www.icourse163.org/newExam/*
// @match        http://www.icourse163.org/learn/*
// @connect      domooc.top
// @connect      *
// @grant        unsafewindow
...
// @antifeature  payment 用户初始50积分消耗完后需要充值才可继续使用答题功能，刷课功能不影响
...
(function() {
    ...
    let usersetting = {
        ...
        timeout: 5000, //答题延时，每两道题之间的时间间隔，单位为毫秒，1秒=1000毫秒，最短为5000
        ...
        $(() => {
            ...
            utils.processReturnResult = function(res) {
                ...
                if (res.user) {
                    ...
                    let info;
                    if (!userleft || !cdkleft) {
                        info = `剩余积分：${userleft + cdkleft}`;
                    } else {
                        info = `剩余积分：${userleft + cdkleft} (CDkey:${cdkleft})`;
                    }
            ...
            function getAnswerByElement(ele, answers) {
                ...
                if (!p || !p.dataset[names.qid]) {
                    ...
                    p.innerHTML =
                        ...
                        domooc.utils.answer2str(answers[p.dataset[names.qid]].answer) :
                        p.innerHTML;
                                                ...
                                                .each((idx2, input) => {
                                                    ...
                                                    if (answer.answer.indexOf(idx2) > -1) {
                                                        ...
                                                        window.setTimeout(() => {
                                                            tempfunc(input, true);
                                                            checkTimeouts();
                                                        }, (checkcnt * (usersetting.timeout - 1000)) / 4);
                                                    }
                                                    checkcnt++;
                                                });
            ...
            let intHandler = window.setInterval(() => {
                ...
                if (
                    ...
                    if (!updatemsg || parseInt(updatemsg) < 1909) {
                        window.setTimeout(() => {
                            view.showVersion();
                ...
                buttons: [{
                    ...
                    },
                    {
                        ...
                        onclick: function() {
                            ...
                            group1.append(
                                `<button class="${names.domooc} button green" onclick="window.open('http://cdn.domooc.top/domoocreadme','_blank')">使用说明</button>`
                            );
                            group1.append(
                                `<button class="${names.domooc} button green" onclick="window.open('http://cdn.domooc.top/domoocinstall','_blank')">更新脚本</button>`
                            );
                  ...
                  _usersetting.timeout / 1000
                          ...
                          x.used + "",
                          4
                        )}  时间：${new Date(x.timestamp).toLocaleString()} `;
                                        });
                                    }
                                    txt = txt + "<br><br>" + txt2;
                                }
            ...
            <div class="u-baseinputui" style="height: 30px;"> <input type="text" id="domoocsearch"
                    ...
                    function showQuizs(jnames, names) {
                        ...
                        if (jnames.length > 0) {
                            jnames.each(function() {
                                var title = domoocformat($(this)[0].innerText);
                                console.log(title);
                                let clear = (text) => {
                                    return $(this)
                                        .text()
                ...
                showVersion() {
                    this.showServerMsg({
                        title: "当前版本：2.0",
                        message: "1. 修复了部分题目不能获取答案的问题" +
                            "<br><br>2. 支持新版考试</span>" +
                            "<br><br>3. 支付时输入邮箱可以通过邮箱发送cdkey",
                    });
                },
                init: function() {
                    let that = this;
